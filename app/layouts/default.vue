      ...
      <div class="mx-auto max-w-[60rem] px-4">
        <div class="flex items-center justify-between py-3">
          <a href="/dashboard" class="flex items-center gap-2">
            <span
              class="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-ink text-white font-bold shadow">W</span>
          ...
          <div v-if="user" class="hidden sm:flex items-center gap-3">
            <div class="flex items-center gap-2 rounded-full border border-slate-200 px-2 py-1 shadow-sm bg-white/80">
              <img 
                :src="user.user_metadata?.avatar_url || 'https://i.pravatar.cc/80?img=8'" 
                alt="avatar" 
            ...
            <button
              @click="handleLogout"
              class="focus-ring rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium hover:bg-slate-50">
              Logout
            </button>
          </div>

          <button
            class="sm:hidden inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 focus-ring bg-white/80"
